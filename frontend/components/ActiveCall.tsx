...
export default function ActiveCall() {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-8 bg-white rounded-2xl shadow-xl border w-full max-w-sm">
      ...
      <div className={`mb-8 w-32 h-32 rounded-full flex items-center justify-center transition-all duration-500 ${
        isConnected ? 'bg-green-100 shadow-[0_0_30px_rgba(34,197,94,0.3)]' : 'bg-gray-100'
      }`}>
      ...
      {!isConnected ? (
        ...
        >
          {isConnecting ? <Loader2 className="animate-spin" /> : <Mic size={18} />}
          Start Conversation
        </button>
      ) : (
        <button
